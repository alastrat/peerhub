...
export default async function SurveyResultsPage({ params }: PageProps) {
  ...
  const [results, company] = await Promise.all([
    getSurveyResults(id, companyId),
    prisma.company.findUnique({ where: { id: companyId }, select: { featureHubs: true } }),
  ]);
  if (!results) notFound();

  const featureHubs = company?.featureHubs ?? false;
  ...
  const teamIds = [...new Set(results.teamBreakdown.map((t) => t.teamId))];
  const teamDimensions = [...new Set(results.teamBreakdown.map((t) => t.dimensionId))];
  const teamNames = new Map(results.teamBreakdown.map((t) => [t.teamId, t.teamName]));
  const teamDimNames = new Map(results.teamBreakdown.map((t) => [t.dimensionId, t.dimensionName]));
  const teamHeatmapLookup = new Map(
    results.teamBreakdown.map((t) => [`${t.teamId}-${t.dimensionId}`, t.average])
  );

  // Build hub heatmap data
  const hubIds = [...new Set(results.hubBreakdown.map((h) => h.hubId))];
  const hubDimensions = [...new Set(results.hubBreakdown.map((h) => h.dimensionId))];
  const hubNames = new Map(results.hubBreakdown.map((h) => [h.hubId, h.hubName]));
  const hubDimNames = new Map(results.hubBreakdown.map((h) => [h.dimensionId, h.dimensionName]));
  const hubHeatmapLookup = new Map(
    results.hubBreakdown.map((h) => [`${h.hubId}-${h.dimensionId}`, h.average])
  );
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Team x Dimension Heatmap */}
      {teamIds.length > 0 && teamDimensions.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Team Heatmap</CardTitle>
            <CardDescription>Average scores by team and dimension (includes cross-department members)</CardDescription>
          </CardHeader>
          <CardContent className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  {teamDimensions.map((dimId) => (
                    <th key={dimId} className="text-center px-3 py-2 font-medium">
                      {teamDimNames.get(dimId)}
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {teamIds.map((teamId) => (
                  <tr key={teamId} className="border-t">
                    <td className="px-3 py-2 font-medium">{teamNames.get(teamId)}</td>
                    {teamDimensions.map((dimId) => {
                      const value = teamHeatmapLookup.get(`${teamId}-${dimId}`);
                      return value != null ? (
                        <HeatmapCell key={dimId} value={value} maxScore={maxScore} />
                      ) : (
                        <td key={dimId} className="px-3 py-2 text-center text-muted-foreground">—</td>
      ...
      {/* Hub x Dimension Heatmap (conditional on featureHubs) */}
      {featureHubs && hubIds.length > 0 && hubDimensions.length > 0 && (
        <Card>
          <CardHeader>
            <CardTitle>Hub Heatmap</CardTitle>
            <CardDescription>Average scores by hub and dimension</CardDescription>
          </CardHeader>
          <CardContent className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr>
                  ...
                  {hubDimensions.map((dimId) => (
                    <th key={dimId} className="text-center px-3 py-2 font-medium">
                      {hubDimNames.get(dimId)}
                    </th>
                  ))}
                </tr>
              </thead>
              <tbody>
                {hubIds.map((hubId) => (
                  <tr key={hubId} className="border-t">
                    <td className="px-3 py-2 font-medium">{hubNames.get(hubId)}</td>
                    {hubDimensions.map((dimId) => {
                      const value = hubHeatmapLookup.get(`${hubId}-${dimId}`);
                      return value != null ? (
                        <HeatmapCell key={dimId} value={value} maxScore={maxScore} />
                      ) : (
                        <td key={dimId} className="px-3 py-2 text-center text-muted-foreground">—</td>
